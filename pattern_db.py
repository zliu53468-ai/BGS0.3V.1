...
# Pattern 樣本保護參數
# ============================================================
# PATTERN_ZERO_SAMPLE_AS_UNAVAILABLE=1：
# pattern_db 回傳 available=False / sample_size=0 時，不再把它視為有效路單命中。
# PATTERN_WEIGHT_REQUIRE_AVAILABLE=1：
# pattern 不可用時，PATTERN_WEIGHT 不參與最終融合，避免中性值稀釋/干擾。
# REQUIRE_PATTERN_SAMPLE_FOR_ENTRY=1：
# 可選風控；若路單樣本為 0，必須達到 MIN_GAP_WITHOUT_PATTERN 才允許進場。
PATTERN_ZERO_SAMPLE_AS_UNAVAILABLE = env_bool("PATTERN_ZERO_SAMPLE_AS_UNAVAILABLE", "1")
PATTERN_WEIGHT_REQUIRE_AVAILABLE = env_bool("PATTERN_WEIGHT_REQUIRE_AVAILABLE", "1")
REQUIRE_PATTERN_SAMPLE_FOR_ENTRY = env_bool("REQUIRE_PATTERN_SAMPLE_FOR_ENTRY", "0")
MIN_GAP_WITHOUT_PATTERN = env_float("MIN_GAP_WITHOUT_PATTERN", "0.150")
...
def extract_pattern_sample_size(rec: Dict[str, Any]) -> int:
    """
    統一取得 pattern record 的樣本數。
    支援 pattern_db.py 回傳 sample_size、sample、no_tie_sample、count、total、n，
    也支援 matched=[{sample: ...}] 的加總格式。
    """
    if not isinstance(rec, dict):
        return 0
...
def is_invalid_pattern_record(rec: Any) -> bool:
    """
    判斷 pattern_db 回傳值是否只是冷啟動/未命中/零樣本 fallback。
    這是修正「路單樣本 0 卻被當成有效 pattern」的核心。
    """
    if not isinstance(rec, dict):
        return True
    ...
    if source in {
        ...
        "PATTERN_NOT_MATCHED",
        "PATTERN_COLD_START",
        "PATTERN_DB_NEUTRAL_FALLBACK",
        "PATTERN_DB_ZERO_SAMPLE_FALLBACK",
    }:
        return True
    ...
    if PATTERN_ZERO_SAMPLE_AS_UNAVAILABLE and extract_pattern_sample_size(rec) <= 0:
        return True
...
def pattern_db_lookup(
    ...
    # pattern_lookup 即使未命中，也可能回傳 dict + 中性機率。
    # 這裡要把 available=False / sample_size=0 的結果視為「未命中」，
    # 才不會出現「路單樣本 0 但 pattern_available=True」的假命中。
    if is_invalid_pattern_record(rec):
        rec = None
    ...
    if not isinstance(rec, dict):
        neutral = neutral_record("PATTERN_DB_ZERO_SAMPLE_FALLBACK")
        neutral["feature_key"] = pattern
        neutral["pattern"] = pattern
        neutral["window"] = 0
        return neutral

    sample_size = extract_pattern_sample_size(rec)

    if PATTERN_ZERO_SAMPLE_AS_UNAVAILABLE and sample_size <= 0:
        neutral = neutral_record("PATTERN_DB_ZERO_SAMPLE_FALLBACK")
        neutral["feature_key"] = rec.get("feature_key", pattern)
        neutral["pattern"] = pattern
        neutral["window"] = int(rec.get("window", 0) or 0)
        return neutral
...
def pattern_feature_db_lookup(player_point: int, banker_point: int) -> Dict[str, Any]:
    ...
    try:
        ...
        sample_size = extract_pattern_sample_size(raw)
...
def predict(player_point: int, banker_point: int, rounds: List[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # 路單層沒有有效樣本時，不讓 PATTERN_WEIGHT 參與融合，
    # 避免「路單樣本 0」仍拿中性機率稀釋/干擾主模型。
    if PATTERN_WEIGHT_REQUIRE_AVAILABLE and (not pattern.get("available") or int(pattern.get("sample_size", 0) or 0) <= 0):
        pat_w = 0.0
    ...
    pattern_sample_size_for_guard = int(pattern.get("sample_size", 0) or 0)
    pattern_available_for_guard = bool(pattern.get("available", False))

    # 可選風控：若路單完全沒有樣本，必須達到更高差距才允許進場。
    # 預設關閉，若要啟用請設定 REQUIRE_PATTERN_SAMPLE_FOR_ENTRY=1。
    if (
        REQUIRE_PATTERN_SAMPLE_FOR_ENTRY
        and entry_allowed
        and (not pattern_available_for_guard or pattern_sample_size_for_guard <= 0)
        and gap < MIN_GAP_WITHOUT_PATTERN
    ):
        entry_allowed = False
        entry_level = "no_entry"
        weak_reason = f"路單樣本不足，且莊閒差距未達 {MIN_GAP_WITHOUT_PATTERN * 100:.1f}%，建議觀察一局"
    ...
    result = {
        ...
        "pattern_zero_sample_as_unavailable": PATTERN_ZERO_SAMPLE_AS_UNAVAILABLE,
        "pattern_weight_require_available": PATTERN_WEIGHT_REQUIRE_AVAILABLE,
        "require_pattern_sample_for_entry": REQUIRE_PATTERN_SAMPLE_FOR_ENTRY,
        "min_gap_without_pattern": MIN_GAP_WITHOUT_PATTERN,
        ...
        "mode": "POINT_DB_PLUS_FEATURE_DB_COMPOSITION_MC_HYBRID_V8_1_PATTERN_SAMPLE_FIXED",
    }
