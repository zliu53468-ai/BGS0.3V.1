...
def analyze_game_screen(
    ...
    with ThreadPoolExecutor(max_workers=2, thread_name_prefix="screen-vision") as executor:
        ...
        road_future = executor.submit(detect_road_sequence_detailed, image_path, venue=str((existing_session or {}).get("venue") or ""), input_type="auto")
    ...
    raw_outcomes = [str(v).upper() for v in list(road.get("raw_outcomes") or sequence) if str(v).upper() in {"B", "P", "T"}]
