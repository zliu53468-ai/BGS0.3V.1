# lgbm_train.py — train LightGBM on Big Road features
...
def main():
    ...
    valid_set=lgb.Dataset(X[va], label=y[va], reference=train_set)
    ...
    bst=lgb.train(params, train_set, num_boost_round=2000, valid_sets=[valid_set],
                  early_stopping_rounds=100, verbose_eval=100)
    os.makedirs(os.path.dirname(LGBM_OUT_PATH), exist_ok=True)
    bst.save_model(LGBM_OUT_PATH)
    print(f"[OK] saved LGBM to {LGBM_OUT_PATH}")
