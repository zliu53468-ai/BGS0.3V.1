"""B/P/T 線上機率校準器（V9.8.1 牌路方向保留版）。

採用保守的預測／實際比例修正，且只有在已結算樣本達門檻後才啟用。
它改善的是機率校準與過度自信問題，不會取得真人桌未公開牌序。

V9.8.1 規則：
- 若前一層已標記 road_direction_primary=True，校準器只調整 B/P/T 機率。
- 「下一局方向評估」保留牌路先行的 recommend，不再被校準後機率改寫。
- 當校準後機率與牌路方向分歧時，只把 action 降為 O（觀望），不改成相反方向。
"""
...
CALIBRATION_MIN_GLOBAL = max(
    50, int(os.getenv("CALIBRATION_MIN_GLOBAL", "250") or "250")
)
CALIBRATION_MIN_VENUE = max(
    50, int(os.getenv("CALIBRATION_MIN_VENUE", "180") or "180")
)
CALIBRATION_MIN_ROOM = max(
    30, int(os.getenv("CALIBRATION_MIN_ROOM", "100") or "100")
)
CALIBRATION_MAX_STRENGTH = max(
    0.0,
    min(
        0.50,
        float(os.getenv("CALIBRATION_MAX_STRENGTH", "0.25") or "0.25"),
    ),
)
TIE_SIGNAL_MIN_SAMPLES = max(
    100, int(os.getenv("TIE_SIGNAL_MIN_SAMPLES", "500") or "500")
)
TIE_SIGNAL_MIN_PROBABILITY = max(
    0.10,
    min(
        0.30,
        float(os.getenv("TIE_SIGNAL_MIN_PROBABILITY", "0.125") or "0.125"),
    ),
)
TIE_SIGNAL_MIN_EDGE = max(
    0.0,
    min(0.15, float(os.getenv("TIE_SIGNAL_MIN_EDGE", "0.015") or "0.015")),
)
MIN_DIRECTION_EDGE = max(
    0.0,
    min(0.20, float(os.getenv("HG_MIN_DIRECTION_EDGE", "0.016") or "0.016")),
)
...
def _best_summary(venue: str, room: str) -> tuple[str, Dict[str, Any], int]:
    scopes = [
        (
            "room",
            get_performance_summary(venue=venue, room=room, limit=5000),
            CALIBRATION_MIN_ROOM,
        ),
        (
            "venue",
            get_performance_summary(venue=venue, room="", limit=5000),
            CALIBRATION_MIN_VENUE,
        ),
        (
            "global",
            get_performance_summary(venue="", room="", limit=10000),
            CALIBRATION_MIN_GLOBAL,
        ),
    ]
...
def _road_direction_is_primary(result: Mapping[str, Any]) -> bool:
    """判斷前一層是否已指定牌路方向為最終顯示方向。"""
    integration = result.get("road_integration")
    integration_map = integration if isinstance(integration, Mapping) else {}
    return bool(
        result.get("road_direction_primary")
        or integration_map.get("road_direction_primary")
        or str(result.get("direction_source") or "").strip().lower()
        == "road_model_primary"
    )
...
def calibrate_prediction(
    ...
    *,
    ...
    active = bool(
        CALIBRATION_ENABLED
        and scope != "none"
        and CALIBRATION_MAX_STRENGTH > 0
    )
    ...
    if active:
        empirical = _normalize(
            dict(summary.get("empirical_probabilities") or BASELINE)
        )
        mean_predicted = _normalize(
            dict(summary.get("mean_predicted_probabilities") or BASELINE)
        )
        maturity = min(
            1.0,
            (sample_count - minimum + 1) / max(1.0, minimum * 3.0),
        )
        ...
        for key in OUTCOMES:
            ratio = max(
                ...
                min(
                    1.80,
                    empirical[key] / max(1e-9, mean_predicted[key]),
                ),
            )
    ...
    road_direction_primary = _road_direction_is_primary(result)
    existing_direction = str(result.get("recommend") or "").upper().strip()
    if existing_direction not in {"B", "P", "T"}:
        existing_direction = probability_direction

    # 牌路主導時，校準器不可再把 B 改成 P 或把 P 改成 B。
    preserved_direction = (
        existing_direction
        if road_direction_primary and existing_direction in {"B", "P"}
        else probability_direction
    )

    tie_ev = t * 8.0 - (b + p)
    tie_break_even = 1.0 / 9.0  # 8:1 賠付的理論損益兩平機率
    tie_signal_allowed = bool(
        not road_direction_primary
        and active
    ...
    # 方向不一致時只觀望；recommend 仍保留牌路方向供面板顯示。
    ...
    if tie_signal_allowed:
        ...
        if road_direction_primary:
            if direction_consistent:
                extra_reason = "校準器保留牌路先行方向，且校準後機率方向一致"
            else:
                extra_reason = "校準器保留牌路先行方向；校準後機率方向分歧，因此降為觀望"
            signal_reason = (
    ...
    result["calibration"] = {
        ...
        "road_direction_primary": road_direction_primary,
        ...
        "direction_preserved": bool(
            road_direction_primary and existing_direction in {"B", "P"}
        ),
