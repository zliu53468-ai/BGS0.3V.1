...
LINUCB_FORGETTING = max(0.70, min(1.0, float(os.getenv("LINUCB_FORGETTING", "0.90") or "0.90")) or 0.90)
...
candidates += [Path("/var/data/contextual_linucb_state.json"), Path(file).resolve().parent / "data" / "contextual_linucb_state.json", Path("/tmp/contextual_linucb_state.json")]
for candidate in candidates:
try:
candidate.parent.mkdir(parents=True, exist_ok=True)
probe = candidate.parent / f".linucb_write{time.time_ns()}"
...
def init(self, alpha: float = LINUCB_ALPHA):
...
# 🎯 【勝率提升重構】：一偵測到和局 'T'，立馬精確攔截！
# 絕對不呼叫 _decay_arms 記憶衰減，直接返回。實施數學上的「時間暫停」，保護大腦不被和局稀釋
if actual == "T":
return {"updated": True, "action": action, "actual_outcome": actual, "reward": 0.0, "directional_sample_applied": False, "forgetting": 1.0, "reason": "tie_is_pure_non_directional_freeze_memory_saved"}
...
# 🎯 【勝率提升重構】：如果是和局，直接在此精確攔截，不呼叫 _update_scope 去觸發隨後的任何狀態更新
resolved_outcome = raw_history[previous_len]
if resolved_outcome == "T":
scope["pending"] = {} # 清空 pending 狀態，準備下一手
return {"updated": True, "history_aligned": True, "resolved_history_index": previous_len, "history_rounds_after_append": len(raw_history), "action": str(pending.get("action") or ""), "actual_outcome": "T", "reward": 0.0, "reason": "pending_tie_intercepted_and_frozen"}
result = self._update_scope(scope, action=str(pending.get("action") or ""), context_vector=deepcopy(list(pending.get("context_vector") or [])), actual_outcome=resolved_outcome)
scope["pending"] = {}
...
root["scopes"][scope_key] = scope
_write_state(root)
# 🎯 【勝率提升重構】：建立不含alpha探勘項的純期望值均值字典 (mean_scores) 傳回決策層
mean_scores = {"B": float(scores["B"]["mean"]), "P": float(scores["P"]["mean"]), "T": 0.0}
return {
...
"probabilities": probabilities, "mean_scores": mean_scores, "selected_win_probability": float(selected_probability), "confidence": float(selected_probability),
