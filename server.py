...
def ema_smooth(session: Dict[str, Any], prob: np.ndarray, outcome: int) -> np.ndarray:
    ...
    if alpha == 0.0 or prev is None:
        session["prob_sma"] = prob
        return prob
    try:
        skip_flag = int(get_stage_param(session, "SKIP_TIE_UPD", str(int(env_flag("SKIP_TIE_UPD", 1)))))
    except Exception:
        skip_flag = 1
    ...
    try:
        ...
        prev_vec = np.asarray(prob, dtype=np.float32)
    new_vec = alpha * prob + (1.0 - alpha) * prev_vec
    s = float(new_vec.sum())
    if s > 0.0:
        new_vec = new_vec / s
    session["prob_sma"] = new_vec
    return new_vec
...
VERSION = "bgs-independent-2025-10-04+stage-smoothing"
...
if __name__ == "__main__":
    ...
    log.info("Starting %s on port %s (CONTINUOUS_MODE=%s, PF_INIT=%s, DEPLETE_OK=%s, MODE=%s, THEO_BLEND=%.3f)", VERSION, port, CONTINUOUS_MODE, pf_initialized, DEPLETE_OK, DECISION_MODE, THEO_BLEND)
    if _flask_available and Flask is not None:
