# -*- coding: utf-8 -*-
"""server.py — Updated version with stage switching and prediction smoothing pipeline
Patched with:
- 前/中/後期分段切換：以手數切段，支援 EARLY_*/MID_*/LATE_* 環境變數覆蓋
- 預測平滑管線：THEO_BLEND + 會話級 EMA 平滑（PROB_SMA_ALPHA）
- 修正 Deplete 參數讀取：DEPLETEMC_SIMS、DEPL_FACTOR 從環境變數讀取
- 支援分段 PF_PRED_SIMS 等動態覆寫
"""
import os
import sys
import logging
...
SKIP_TIE_UPD = int(os.getenv("SKIP_TIE_UPD", "1"))
...
def get_session(uid: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "last_outcome": None, "hand_count": 0, "smoothed_prob": None,
    }
...
# ---------- 新增：階段切換功能 ----------
def get_stage_params(hand_count: int) -> Dict[str, Any]:
    """
    根據手數決定階段並返回對應的環境變數值
    early: 0-20手, mid: 21-60手, late: 61+手
    """
    if hand_count <= 20:
        stage = "EARLY"
    elif hand_count <= 60:
    ...
    log.debug("階段 %s (手數 %d) 參數: %s", stage, hand_count, params)
    return params
...
def apply_session_ema_smoothing(current_prob: np.ndarray, session: Dict[str, Any], outcome: int) -> np.ndarray:
    ...
    prev_smoothed = session.get("smoothed_prob")
    ...
    # 更新會話中的平滑機率
    session["smoothed_prob"] = smoothed.tolist()
    return smoothed
...
def _handle_points_and_predict(sess: Dict[str, Any], p_pts: int, b_pts: int, reply_token: str):
    ...
    try:
        ...
        if LOG_DECISION or SHOW_CONF_DEBUG:
            ...
            log.debug("平滑流程: 原始→理論混合→EMA = %s → %s → %s", 
                     np.round(p, 4), np.round(p_theo, 4), np.round(p_final, 4))

        log.info("完整處理完成, 總耗時: %.2fs (手數: %d)", time.time() - start_time, hand_count)
    except Exception as e:
        ...
        def on_text(event):
            ...
            try:
                ...
                if text == "遊戲設定" or up == "GAME SETTINGS":
                    ...
                    sess["hand_count"] = 0; sess["smoothed_prob"] = None  # 重置手數和平滑
                    # 顯示剩餘分鐘（持久鎖）
                    first_ts = _rget(_trial_key(uid, "first_ts"))
                    if first_ts:
...
if __name__ == "__main__":
    ...
    log.info("Starting %s on port %s (CONTINUOUS_MODE=%s, PF_INIT=%s, DEPLETE_OK=%s, MODE=%s, THEO_BLEND=%.3f, PROB_SMA_ALPHA=%.2f, SKIP_TIE_UPD=%s)",
             VERSION, port, CONTINUOUS_MODE, pf_initialized, DEPLETE_OK, DECISION_MODE, THEO_BLEND, PROB_SMA_ALPHA, SKIP_TIE_UPD)
    if _flask_available and Flask is not None:
