...
PF_UPD_SIMS  = int(os.getenv("PF_UPD_SIMS", "80"))   # backend=mc 使用；exact 不用
PF_PRED_SIMS = int(os.getenv("PF_PRED_SIMS", "0"))   # backend=mc 使用；exact 不用
...
def update_outcome_api():
    ...
    if o in ("B","莊","0"):
        ...
        PF.update_outcome(1)
    elif o in ("T","和","TIE","DRAW","2"):
        PF.update_outcome(2)
    else:
        return jsonify(ok=False, msg="outcome 必須是 B/P/T 或 莊/閒/和"), 400
...
def predict_api():
    ...
    if not last_text and lo:
        if lo in ("B","莊","0"):
            PF.update_outcome(0); last_text = "上局結果: 莊勝"
        elif lo in ("P","閒","1"):
            PF.update_outcome(1); last_text = "上局結果: 閒勝"
        elif lo in ("T","和","TIE","DRAW","2"):
            PF.update_outcome(2); last_text = "上局結果: 和局"
