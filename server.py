...
# -*- coding: utf-8 -*-
"""
BGS LINE Bot — v13.1 BigRoad Grid (6x20) + T-aware + PH drift + Regime/Momentum
更新點：
- 未開始分析前：每次輸入都即時回覆 B/P/T 統計與近20手（預設不再等 3 手）
- 新增【返回】按鈕（UNDO）：僅在尚未開始分析前可用，用於刪除最後一手以核對歷史
- 其他沿用 v13：6×20 大路專家、PH 漂移、Regime/Momentum、T 機率校正、/tmp 寫入
"""
...
# LINE 前置控制
# 0=完全不回；>=1 表示每 N 手回一次。為了「每手都回」，預設設為 1。
PRESTART_EVERY_N = int(os.getenv("PRESTART_EVERY_N","1"))
...
def bigroad_expert(seq: List[str]) -> List[float]:
    ...
    cont = 0.50
    cont += 0.10 * min(4.0, float(h))
    cont += 0.06 * ladder_score
    cont -= 0.10 * min(1.0, turn_rate)
    cont -= 0.08 * min(1.0, single_rate*1.2)
    cont -= 0.05 * min(1.0, double_rate)
    cont -= 0.06 * min(1.0, std3/3.0)
    if filled: cont += 0.04
    try:
...
def ensemble_with_anti_stuck(seq: List[str], weight_overrides: Optional[Dict[str,float]]=None) -> List[float]:
    ...
    BIGR_W  =float(os.getenv("BIGR_W","0.18"))
    ...
    probs=B(probs,p_bigr,BIGR_W)   # 6x20 大路融合
    # 交錯期翻邊偏置
...
def health(): return jsonify(status="healthy", version="v13.1-bigroad-grid+undo")
...
def flex_buttons_card() -> 'FlexSendMessage':
    # 注意：避免因 color 放在不支援屬性的位置而報 400，這裡只在 primary 按鈕上用 color
    contents={
        "type":"bubble",
        "body":{
            "type":"box","layout":"vertical","spacing":"md",
            "contents":[
                {"type": "text", "text": "🤖 請先把『歷史牌局』輸入完畢，再按【開始分析】！", "wrap": True, "size": "sm"},
...
if USE_LINE and handler is not None:
    ...
    def handle_postback(event):
        ...
        if data=="UNDO":
            # 僅在尚未開始分析前允許返回
            if not ready and seq:
                removed = seq.pop()
                USER_HISTORY[uid]=seq
                b,p,t=bpt_counts(seq); s="".join(seq[-20:]) if seq else "（空）"
                msg=(f"已返回（刪除最後一手：{removed}）。\n"
                     f"目前 {len(seq)} 手；近20：{s}\n"
                     f"統計：B={b}｜P={p}｜T={t}\n繼續輸入或按【開始分析】。")
            else:
                msg="返回僅在『尚未開始分析』時可用。"
            line_bot_api.reply_message(event.reply_token,
        ...
        if data not in CLASS_ORDER:
            line_bot_api.reply_message(event.reply_token,
                [TextSendMessage(text="請用按鈕輸入（莊/閒/和/返回），或選開始/結束分析。", quick_reply=quick_reply_bar()),
                 flex_buttons_card()])
            return
        ...
        # 未開始分析：即時回覆（每 N 手回一次；預設 N=1）
        if not ready:
            if PRESTART_EVERY_N==0 or (len(seq)%max(1,PRESTART_EVERY_N)==0):
                b,p,t=bpt_counts(seq); s="".join(seq[-20:]) if seq else "（空）"
                msg=(f"已記錄 {len(seq)} 手（近20：{s}）\n"
                     f"目前統計：B={b}｜P={p}｜T={t}\n"
                     f"👉 如需修正可按【返回】；全部輸入完畢後再按【開始分析】")
                line_bot_api.reply_message(event.reply_token,
                    [TextSendMessage(text=msg, quick_reply=quick_reply_bar()), flex_buttons_card()])
            return
