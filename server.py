...
# Version: bgs-pf-continuous-2025-09-17-ka5
...
def _rget(k: str) -> Optional[str]:
    try: return redis_client.get(k) if redis_client else None
    except Exception as e: log.warning("[Redis] GET err: %s", e); return None

def _rset(k: str, v: str, ex: Optional[int] = None):
    try:
        if redis_client: redis_client.set(k, v, ex=ex)
    except Exception as e: log.warning("[Redis] SET err: %s", e)

def _rsetnx(k: str, v: str, ex: int) -> bool:
    try:
        if redis_client: return bool(redis_client.set(k, v, ex=ex, nx=True))
        # fallback
        if k in SESS_FALLBACK: return False
...
def trial_left_minutes(sess: Dict[str, Any]) -> int:
    ...
    now = int(time.time()); used = (now - int(sess.get("trial_start", now))) // 60
...
def decide_only_bp(prob: np.ndarray) -> Tuple[str, float, float, str]:
    ...
    if final_edge < EDGE_ENTER: return ("觀望", final_edge, 0.0, "⚪ 優勢不足")
    if USE_KELLY:
        if side == 0:
            b = 0.95; f = KELLY_FACTOR * ((pB * b - (1 - pB)) / b)
        else:
            b = 1.0;  f = KELLY_FACTOR * ((pP * b - (1 - pP)) / b)
        bet_pct = min(MAX_BET_PCT, max(0.0, float(f))); reason = "¼-Kelly"
    else:
        bet_pct = 0.25 if final_edge>=0.10 else 0.15 if final_edge>=0.07 else 0.10 if final_edge>=0.04 else 0.05
        reason = "階梯式配注"
...
def format_output_card(prob: np.ndarray, choice: str, last_pts_text: Optional[str], bet_amt: int, cont: bool) -> str:
    ...
    if cont:
        block.append("\n📌 連續模式已開啟：請直接輸入下一局點數（例：65 / 和 / 閒6莊5）")
    return "\n".join(header + [""] + block)
...
@app.get("/health"); def health(): return jsonify(ok=True, ts=time.time(), version=VERSION), 200
@app.get("/healthz"); def healthz(): return jsonify(ok=True, ts=time.time(), version=VERSION), 200
...
def _handle_points_and_predict(sess: Dict[str,Any], p_pts: int, b_pts: int, reply_token: str):
    ...
    if p_pts == b_pts:
        ...
        try:
            if int(os.getenv("SKIP_TIE_UPD","0")) == 0: PF.update_outcome(2)
    ...
    try:
        ...
        def on_follow(event):
            ...
            _reply(event.reply_token,
                   f"👋 歡迎！請輸入『遊戲設定』開始；已開啟連續模式，之後只需輸入點數（例：65 / 和 / 閒6莊5）即可自動預測。")
            ...
            try:
                ...
                if phase == "choose_game":
                    ...
                    if text.isdigit() and int(text) > 0:
                        ...
                        _reply(event.reply_token, f"👍 已設定本金：{sess['bankroll']:,}\n📌 請直接輸入上局點數（例：65 / 和 / 閒6莊5）即可自動預測。")
                        save_session(uid, sess); return
                    else:
                        _reply(event.reply_token, "❌ 金額格式錯誤，請直接輸入正整數（例：5000）"); return
