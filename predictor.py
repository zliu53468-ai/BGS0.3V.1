...
def predict(history: Union[str, Iterable[Any], None] = None, venue: str = "", room: str = "", shoe_id: str = "", user_id: str = "", run_seed: Optional[int] = None, shoe_context: Optional[Mapping[str, Any]] = None, road_context: Optional[Mapping[str, Any]] = None) -> Dict[str, Any]:
    ...
    # 🎯 【勝率提升重構】：優先提取不含探勘項的純期望值均值 (mean_scores)，用以重新校準概率空間
    mean_scores = policy.get("mean_scores")
    if isinstance(mean_scores, Mapping) and "B" in mean_scores and "P" in mean_scores:
        ...
        calibrated_probs = {"B": exp_b / (exp_b + exp_p), "P": exp_p / (exp_b + exp_p), "T": 0.0}
        probabilities, direction, _ = _normalize_policy_probabilities(policy)
    ...
    if pure_ev > 0.0:
        ...
        money = {}
        final_ratio = float(MIN_BET_RATIO)  # 純淨 EV 為負時，立刻縮回 5% 最小底注進行大盤震盪防守
        
    bet_percentage = final_ratio * 100.0
    money.update({"final_bet_ratio": final_ratio, "bet_percentage": bet_percentage, "bet_amount": bankroll * final_ratio, "bet_allowed": True, "mandatory_bet": True, "virtual_ev": pure_ev, "virtual_ev_percent": pure_ev * 100.0})
    ...
    shoe_returns = dict(shoe.get("expected_returns") or {}); banker_ev = shoe_returns.get("B") if shoe_available else None; player_ev = shoe_returns.get("P") if shoe_available else None; selected_ev = pure_ev; edge = float(money.get("edge", 0.0) or 0.0)
...
context_meta.update({"formal_direction_source": formal_source, "primary_model": "CONTEXTUAL_LINUCB", "linucb_direction_weight": 1.0, "road_direction_weight": 0.0, "road_context_direction_weight": 0.0, "shoe_context_used_for_formal_direction": True, "shoe_context_used_as_linucb_features": True, "shoe_independent_direction_vote": False, "exact_shoe_composition_used_for_direction": False, "card_composition_direction_weight": 0.0, "derived_road_independent_vote": False, "geometry_independent_vote": False, "anti_echo_external_penalty": False, "lstm_direction_weight": 0.0, "fallback_markov_direction_weight": 0.0, "remaining_cards": remaining_cards, "remaining_ratio": remaining_ratio, "penetration": penetration, "shoe_stage": shoe_stage, "cut_progress": cut_progress, "target_hands_min": int(TARGET_HANDS_MIN), "target_hands_max": int(TARGET_HANDS_MAX)})
...
"shoe_context_used_for_formal_direction": True, "shoe_context_used_as_linucb_features": True, "exact_shoe_composition_used_for_direction": False,
...
"contextual_bandit_enabled": True, "contextual_bandit_update_enabled": True, "linucb_enabled": True, "linucb_diagnostic_only": False,
...
"probability_semantics": "calibrated_exploit_only_mean_logits_mapping",
"dynamic_prediction_policy": {"version": POLICY_VERSION, "road_primary": False, "linucb_primary": True, "lstm_primary": False, "fallback_active": False, "big_road_rounds": len(big_road), "forecast": policy, "penalty_observe": {"active": False, "force_observe": False}, "exact_card_dependency": False, "exact_card_used_when_available": composition_source in {"remaining_counts", "observed_cards"}, "shoe_probability_decision_weight": 0.0, "ocr_or_screen_flow_modified": False, "formal_direction_source": formal_source, "card_composition_source": composition_source, "shoe_direction_authority": False, "shoe_context_as_features": True, "cut_progress": cut_progress},
...
if "mean_scores" in policy: result["mean_scores"] = dict(policy["mean_scores"])
return result
