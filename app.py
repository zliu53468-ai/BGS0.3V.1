...
# Tie 橫線偵測
HOUGH_MIN_LEN_RATIO = float(os.getenv("HOUGH_MIN_LEN_RATIO", "0.50"))  # ROI 寬度比例(略提高，降低誤判)
...
def _has_horizontal_line(roi_bgr: np.ndarray) -> bool:
    ...
    for x1, y1, x2, y2 in lines[:, 0, :]:
        if abs(y2 - y1) <= max(2, int(h * 0.12)):  # 更嚴格的近水平
            return True
    return False
...
def extract_sequence_from_image(img_bytes: bytes) -> List[str]:
    ...
    try:
        ...
        def cc_blobs(mask, label):
            ...
            min_area = max(90, int(area_med * 0.40))      # 更嚴，過小點去除
            max_area = int(area_med * 7.5) if area_med > 0 else 999999
        ...
        # 同欄/同格去重用兩個閥值：水平間距 + 垂直近似（避免同一欄上下兩顆被當不同手）
        min_gap_x = max(med_w * 0.60, 10)
        min_gap_y = max(med_w * 0.35, 6)
        ...
        for x,y,w0,h0,cx,cy,label in items:
            ...
            cell = (col, row)
            # 允許小幅偏移的去重（避免同一格多次計算）
            near_hit = any((col+dx, row+dy) in seen_cells for dx in (0,1) for dy in (-1,0,1))
            if near_hit:
                continue
...
def _smooth_and_hysteresis(uid: str, probs: Dict[str, float]) -> Dict[str, float]:
    ...
    side_now = "B" if smoothed["banker"] >= smoothed["player"] else "P"
    ...
    if side_last is not None:
        if diff < KEEP_SIDE_MARGIN:
            ...
            if side_last == "B":
                ...
                smoothed["player"] = 1.0 - smoothed["banker"] - smoothed["tie"]
            else:
                smoothed["player"] = max(smoothed["player"], smoothed["banker"])
                smoothed["banker"] = 1.0 - smoothed["player"] - smoothed["tie"]
        else:
            # 想換邊但差距不夠大 → 仍保持
            if (side_last == "B" and smoothed["player"] > smoothed["banker"] and diff < FLIP_GUARD) or \
               (side_last == "P" and smoothed["banker"] > smoothed["player"] and diff < FLIP_GUARD):
                # 壓回上次一邊
                if side_last == "B":
                ...
                # 正規化
                s = smoothed["banker"] + smoothed["player"] + smoothed["tie"]
                for k in smoothed:
                    smoothed[k] = max(0.0, min(1.0, smoothed[k]/s))
